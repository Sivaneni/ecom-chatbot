...
from retreiver.retrieval import Retriever
...
class AgenticRAG:
    ...
    def _save_workflow(self, filename: str = "workflow_mcp.mmd"):
        """Save the workflow graph to a Mermaid file."""
        if os.path.exists(filename):
            print("Workflow already exists")
            return "file already exists"
        else:
            mermaid_code = self._build_workflow().compile().get_graph().draw_mermaid()
            with open(filename, "w") as f:
                f.write(mermaid_code)
            print(f"Workflow saved as Mermaid file: {filename}")
...
if __name__ == "__main__":
    ...
    answer = rag_agent.run("What is the price of DELL G15-5530?")
    print("\nFinal Answer:\n", answer)
